...
export function Select({ value, onChange, options, id, describedBy, placeholder, disabled, label }: SelectProps) {
  return (
    <RSelect.Root value={value} onValueChange={onChange} disabled={disabled}>
      <RSelect.Trigger
        id={id}
        className={`${inputClasses()} flex flex-row items-center flex-nowrap min-w-0 gap-2 justify-between text-left cursor-pointer`}
        aria-describedby={describedBy}
        aria-label={label}
      >
        <span className="truncate min-w-0 flex-1 block text-left">
    ...
    >
      <RPopover.Trigger asChild>
        <button
          ...
          className={clsx(
            inputClasses({ mono }),
            'flex flex-row items-center flex-nowrap min-w-0 gap-2 justify-between text-left cursor-pointer',
          )}
        >
          <span className={clsx('truncate min-w-0 flex-1 block text-left', !value && 'pouf-combobox__placeholder')}>
            {value || placeholder}
          </span>
          <span className="flex-none opacity-60">
            <Icon name="expand" size="sm" />
